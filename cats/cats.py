...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    return lambda p: any([x in split(lower(remove_punctuation(p))) for x in subject])
    # END PROBLEM 2
    ...
    if typed_words == [] and source_words == []:
        ...
        for i in range(min(len(typed_words), len(source_words))):
            if typed_words[i] == source_words[i]:
                total += 1
        return total / min(len(typed_words), len(typed_words)) * 100.0
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    return len([x for x in typed]) / 5 / elapsed * 60.0
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if (diff <= limit and rate == -1) or (diff < rate):
            rate = diff
            correct_word = word
    if correct_word == '':
        return typed_word
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(n):
        if typed[i] == prompt[i]:
            ...
            break
    ret = {'id': user_id, 'progress': point/d}
    upload(ret)
    return point/d
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [x[1:] for x in times_per_player]
    for i in range(len(times_per_player)):
        for k in range(len(times_per_player[0]) - 1):
            times[i][k] = times_per_player[i][k + 1] - times_per_player[i][k]
...
def fastest_words(match):
    ...
    for j in word_indices:
        ...
        t = times[0][j]
        k = 0
        while k < len(get_all_times(match)):
            if t > times[k][j]:
                t = times[k][j]
                user_id = k
            k += 1
        ret[user_id].append(words[j])
    return ret
