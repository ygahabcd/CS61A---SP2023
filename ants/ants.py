...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if not place.ant.is_container and self.is_container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            ...
            if bee is not None and self.lower_bound <= dist <= self.upper_bound:
                return bee
            dist += 1
    ...
    def insects_in_front(self):
        ...
        while current and not current.is_hive:
            if current.ant and current.ant is not self:
                in_front_insects[current.ant] = distance
                if current.ant.is_container and current.ant.ant_contained:
                    in_front_insects[current.ant] = distance
            if current.bees:
                for bee in current.bees:
                    in_front_insects[bee] = distance
            distance += 1
            current = current.entrance
        return in_front_insects
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        if self.damage - 0.25*distance - 0.0625*self.insects_shot > 0:
